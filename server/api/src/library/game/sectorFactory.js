...
const { rndInt, rndDouble, selectByChance, selectByWeight } = require('../helpers');
...
const generateSector = ({ galaxy, sector: { x, y, z, isHub } }) => {
    ...
    // TODO add wormhole as astronomical Object below
		/*{
      type: 'wormhole',
			class: 'WH', // wormhole
			radius: rndDouble(1, 1),
  ...
  let systemType = selectByWeight(types);
...
const generatePlanets = (count) => {
  ...
  let distanceFromStar = rndDouble(5.0, 9.0); // in AU
  ...
  for (let i = 0; i < count; i++) {
    ...
    let object = selectByWeight(objectTypes);

    if (object.object === 'asteroidbelt'){
      if (asteroidBeltCount < 4) {
        object.asteroidsNum = Math.max(rndInt(200, 5000), 0);
        if (distanceFromStar <= 27) { // don't want to be to close to star
          distanceFromStar += rndDouble(2.0, 9.0); 
        }
        asteroidBeltCount++;
      } else {
        do {
    ...
    object.distance = distanceFromStar; // in KM
    
    distanceFromStar += rndDouble(2.0, 9.0); // in KM
